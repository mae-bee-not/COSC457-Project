...
CREATE TABLE labor_line(
  labor_line_id INTEGER PRIMARY KEY,
  order_id INTEGER NOT NULL REFERENCES repair_order(order_id),
  service_id INTEGER NOT NULL REFERENCES service(service_id),
  mechanic_id INTEGER NOT NULL REFERENCES employee(employee_id),
...
CREATE TABLE part_line(
  part_line_id INTEGER PRIMARY KEY,
  order_id INTEGER NOT NULL REFERENCES repair_order(order_id),
...
CREATE TABLE inventory_movement(
  ...
  part_id INTEGER NOT NULL REFERENCES part(part_id),
  order_id INTEGER REFERENCES repair_order(order_id),
...
CREATE TABLE invoice(
  ...
  order_id INTEGER NOT NULL UNIQUE REFERENCES repair_order(order_id),
...
CREATE TRIGGER movement_no_update BEFORE UPDATE ON inventory_movement
BEGIN
  SELECT RAISE(ABORT, 'Inventory ledger is append-only');
END;

CREATE TRIGGER movement_no_delete BEFORE DELETE ON inventory_movement
BEGIN
  SELECT RAISE(ABORT, 'Inventory ledger is append-only');
END;
...
BEGIN
  SELECT RAISE(ABORT, 'Invoice is immutable');
END;
...
BEGIN
  SELECT RAISE(ABORT, 'Payment is immutable');
END;
...
WHEN NOT EXISTS (
  ...
  FROM employee
  WHERE
    employee_id = NEW.mechanic_id
    AND role = 'MECHANIC'
    AND active = 1
)
BEGIN
  SELECT RAISE(ABORT, 'Select an active mechanic');
END;

CREATE TRIGGER order_frozen BEFORE UPDATE ON repair_order
WHEN OLD.status IN ('COMPLETED', 'CANCELLED')
BEGIN
  SELECT RAISE(ABORT, 'Final orders cannot be edited');
END;
...
WHEN (SELECT status FROM repair_order WHERE order_id = NEW.order_id)
!= 'IN_PROGRESS'
BEGIN
  SELECT RAISE(ABORT, 'Order must be authorized and in progress');
END;

CREATE TRIGGER labor_line_no_update BEFORE UPDATE ON labor_line
BEGIN
  SELECT RAISE(ABORT, 'Remove and re-add a draft line to correct it');
END;

CREATE TRIGGER labor_line_delete_guard BEFORE DELETE ON labor_line
WHEN (SELECT status FROM repair_order WHERE order_id = OLD.order_id) NOT IN (
  'OPEN',
  'IN_PROGRESS'
)
BEGIN
  SELECT RAISE(ABORT, 'Final order lines cannot be removed');
END;
